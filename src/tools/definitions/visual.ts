...
export const VISUAL_TOOL_DEFINITIONS: GodotToolDefinition[] = [
  ...
  {
    name: 'physics',
    description: 'Create, inspect, configure, template, and analyze physics bodies, areas, collision layers, and shapes',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: { type: 'string', description: 'Path to the Godot project directory' },
        ...
        action: {
          ...
          description: 'Physics action. Defaults to list.',
        },
        parentNodePath: { type: 'string', description: 'Parent node path for create. Defaults to root.' },
        nodePath: { type: 'string', description: 'Existing physics node path for set_layers or get_collision_info' },
        nodeType: {
          type: 'string',
        ...
        shapeResourcePath: { type: 'string', description: 'Project-relative .tres/.res path for create_shape_resource or reusable template shapes' },
        templateName: {
          type: 'string',
          enum: ['area_trigger_2d', 'area_trigger_3d', 'character_controller_2d', 'character_controller_3d'],
          description: 'Reusable physics template name for template actions.',
        },
        collisionLayer: { type: 'number', description: 'Numeric collision_layer bitmask for CollisionObject nodes' },
        collisionMask: { type: 'number', description: 'Numeric collision_mask bitmask for CollisionObject nodes' },
        collisionLayerNames: { type: 'array', items: { type: 'string' }, description: 'Named physics layers to convert into a collision_layer bitmask' },
        collisionMaskNames: { type: 'array', items: { type: 'string' }, description: 'Named physics layers to convert into a collision_mask bitmask' },
        dimensions: { type: 'object', description: 'Shape dimensions, such as { "width": 32, "height": 32 } or { "x": 1, "y": 2, "z": 1 }' },
        radius: { type: 'number', description: 'Circle, sphere, or capsule radius' },
  ...
  {
    name: 'navigation',
    description: 'Create, inspect, configure, bake, query, and debug NavigationRegion and NavigationAgent nodes',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: { type: 'string', description: 'Path to the Godot project directory' },
        ...
        action: {
          type: 'string',
          enum: ['create', 'list', 'set_polygon', 'configure_bake', 'bake_navigation_mesh', 'query_path', 'create_debug_geometry'],
          description: 'Navigation action. Defaults to list.',
        },
        parentNodePath: { type: 'string', description: 'Parent node path for create. Defaults to root.' },
        ...
        nodePath: { type: 'string', description: 'Navigation node path for set_polygon, bake configuration, path query, or debug geometry' },
        points: { type: 'array', items: { type: 'object' }, description: '2D points for NavigationRegion2D polygon vertices' },
        agentRadius: { type: 'number', description: 'NavigationMesh agent radius for bake configuration' },
        cellSize: { type: 'number', description: 'NavigationMesh bake cell size' },
        cellHeight: { type: 'number', description: 'NavigationMesh bake cell height' },
        startPosition: { type: 'object', description: 'Path query start position as Vector2/Vector3 JSON' },
        endPosition: { type: 'object', description: 'Path query end position as Vector2/Vector3 JSON' },
        debugNodeName: { type: 'string', description: 'Line2D debug node name for create_debug_geometry. Defaults to NavigationDebugPath.' },
