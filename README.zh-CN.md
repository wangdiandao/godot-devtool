...
[![Version](https://img.shields.io/badge/version-3.1.0-blue.svg)](CHANGELOG.zh-CN.md)
...
本仓库也按 IDE 插件包方式组织，供 Agent 编程工具直接读取：

- [plugin.json](plugin.json) 暴露共享插件 metadata、`agents/` 和 `skills/`。
- [.codex-plugin/plugin.json](.codex-plugin/plugin.json) 暴露 Codex marketplace/interface metadata。
- [.claude-plugin/plugin.json](.claude-plugin/plugin.json) 暴露 Claude Code 插件 metadata。
- [agents/godot-dev.agent.md](agents/godot-dev.agent.md) 是可由用户调用的 `godot-dev` Agent。它会通过 [skills/godot-devtool/SKILL.md](skills/godot-devtool/SKILL.md) 和各个聚焦 workflow Skill 路由任务。

`npm run build` 会把这些 manifest、Agent 和全部 Skill 复制到 `build/`，因此 release zip 同时包含 MCP server 和 IDE 风格的引导层。

## 从本地构建安装

1. 从源码构建，或把已发布的 release zip 解压到稳定路径。本版本的 release asset 名称是 `godot-devtool-build-3.1.0.zip`。
