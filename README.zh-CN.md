...
[![Version](https://img.shields.io/badge/version-2.7.0-blue.svg)](CHANGELOG.zh-CN.md)
...
`godot-devtool` 是面向 Godot 4 的 MCP server，用于让 AI 助手检查、编辑、验证和自动化运行中的 Godot 项目。2.7.0 增加通过内置插件执行的实时 editor 场景修改，这些修改进入 Godot UndoRedo 栈，并把 `GDT` dock 重新设计为 editor/runtime 就绪状态面板，不再依赖外部磁盘重载提示。
