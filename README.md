...
`godot-devtool` is a Godot 4 MCP server for AI-assisted project inspection, editing, validation, and runtime automation. Version 2.6.3 fixes Steam Godot headless launches by routing Godot logs to an explicit writable file before running headless operations.
...
1. Download the release build:
   ...
   [godot-devtool-build-2.6.3.zip](https://github.com/wangdiandao/godot-devtool/releases/download/v2.6.3/godot-devtool-build-2.6.3.zip)

2. Extract it to a stable path, for example:

   ```powershell
   Expand-Archive ".\godot-devtool-build-2.6.3.zip" "E:\godot-devtool" -Force
   ```
