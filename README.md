...
`godot-devtool` is a Godot 4 MCP server for AI-assisted project inspection, editing, validation, and runtime automation. Version 2.7.3 hardens stable MCP/WebSocket startup by loading project bridge auth during hello, documents that the MCP server must stay running for live bridge routes, and keeps the 2.7 dock/runtime fixes.
